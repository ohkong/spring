...
<form name="searchform" method="get" action="menu.shop" >
...
<form name="f" method="post" action="rcp.shop">
	...
	<tr>
		...
		<th>정보</th>
		<th style="width: 30px;" class="hover">▲cal</th>
		<th style="width: 30px;" class="hover">▲car</th>
		<th style="width: 30px;" class="hover">▲pro</th>
		<th style="width: 30px;" class="hover">▲fat</th>
	</tr>
	</thead>
	<tbody>
...
<c:forEach items="${list}" var="item" >
	<tr>
	<td><a href="rcp.shop?no=${item.no}"><img src="${item.food_pic}" width="100" height="100"></a></td>
	<td><a href="rcp.shop?no=${item.no}"  style="color: 'blue'">${item.name }</a><br>${item.raw}</td>
	<td>${ item.calorie}</td>
	<td>${ item.carbohyd}</td>
	<td>${ item.protein}</td>
	<td>${ item.fat}</td>
	...
	$(" th").click(function(){
		...
		else if($(this).text()=="▼cal"){
			$(this).text("▲cal")	
		}
		else if($(this).text()=="▲car"){
			$(this).text("▼car")	
		}		
		else if($(this).text()=="▼car"){
			$(this).text("▲car")	
		}
		else if($(this).text()=="▲pro"){
			$(this).text("▼pro")	
		}		
		else if($(this).text()=="▼pro"){
			$(this).text("▲pro")	
		}
		else if($(this).text()=="▲fat"){
			$(this).text("▼fat")	
		}		
		else if($(this).text()=="▼fat"){
			$(this).text("▲fat")	
	...
	$(function(){
		...
		$(".sel_food").click(function(){
			...
			$(".sel_food_text").toggle("fast") //show() ,hide() 번가라가며 실행
			if($(this).text() =="상세검색하기"){
				$(this).text("상세검색 숨기기")
			}else{
				$(this).text("상세검색하기")
